...
def get_parsed_ant_positions(
    ...
    Filter in order to accommodate sparsely sampled data.
    ...
    if gps_df is None:
        ...
        pos_twtt_df = pos_twtt_df.rename(
            columns={"ant_x1": "ant_x", "ant_y1": "ant_y", "ant_z1": "ant_z"}
        )
        ...
        pos_twtt_df = pd.concat(
            [
                ...
                raw_df.merge(
                    gps_df.rename(columns={constants.TIME_J2000: constants.DATA_SPEC.tx_time}),
                    on=constants.DATA_SPEC.tx_time,
                ),
            ],
            ignore_index=True,
        )
        ...
        # To accommodate this, we assume that the INS position offsets between
