...
to the same interrogation if they each receive the ping.

GNATSS supports multiple formats for the two-way travel time input files, which
may be designated by defining a `format` key in the configuration file.
Supported formats are:

- `DFOP`: `DFOP00.raw` files generated by the Sonardyne GPS-A payload
- `legacy`, no value: Custom text-column format with the following columns:
...
Each row in this file corresponds to a ping, containing UTC timestamps of the
ping interrogation and two-way travel times in microseconds. Each of the two-way
travel time columns corresponds to the replies from a specific transponder as
defined in the config.yaml file.

In order to prevent interference between the replies, each transponder may be
assigned an internal delay time that will elapse before sending a reply to an
interrogation ping. These delay times must be defined in the config.yaml file
and will be removed during positioning.
...
column during this time. GNATSS will perform this operation automatically if
two-way travel time files are provided in the `DFOP` format.
...
user's preference, which may be designated by setting a `format` key in the
configuration file. Supported formats are:

- `CSRS`: .pos file generated by the
  [CSRS-PPP](https://webapp.csrs-scrs.nrcan-rncan.gc.ca/geod/tools-outils/ppp.php?locale=en)
  online tool offered by the Canadian Geodetic Survey
- `PRIDE`: kin file generated by
  [PRIDE PPP-AR](https://github.com/PrideLab/PRIDE-PPPAR)
- `legacy`, no value: Custom text-column format with the following columns:
...
The columns of the legacy file format are as follows:
...
The LRI model SV-3 Wave Glider telemeters small extracts of data to shore
subsampled at a 2-minute sampling rate for quallity control purposes. These data
are stored in zipped tarballs, each of which contains a set of .pin files. Each
.pin file contains the data recorded during one ping stored in a JSON structure,
which includes:

- The real-time position and orientation at ping interrogation
- The real-time position and orientation recorded reasonably close (within
  <0.2s) to each of the ping replies
- The times at ping interrogate and reply
- The ranges to each transponder
- The NovAtel [_#RANGEA_](https://docs.novatel.com/OEM7/Content/Logs/RANGE.htm)
  record for the interrogation epoch

Because these files only contain a subsample of the data set, they are only used
for an alternative pre-processing mode using the `--parsed` flag instead of the
default `--posfilter` flag. (See
[_"Real-time" Processing_](parsed_processing.md).)
